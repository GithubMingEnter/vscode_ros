...
wps=np.loadtxt("/home/lwm/vscode_ros/src/create/script/track/line.txt")
...
class MPC(object):
    def __init__(self):
        ...
        m.wg= Param(RangeSet(0,3),initialize={0:1.,1:10.,2:100.,3:1000},mutable=True)#mutable表示是可变的参数
        ...
        m.f=Var(m.sk)
        m.psides=Var(m.sk)
        m.uv=Var(m.uk,bounds=(-0.01,2.0))#bound velocity
        ...
        m.cteobj=m.wg[3]*sum((m.s[3,k]-m.ref_cte)**2 for k in m.sk)#why wg[3]
...
class MpcTracking():
    def __init__(self):
        ...
        self.base_toRobot=rospy.get_param('~baseL',default='map')
    ...
    def getrobotpose(self):
        try:
            (trans,rot)=self.listener.lookupTransform(self.base_toRobot,self.base_frame,rospy.Time(0))
    ...
    def pub_ref_path(self,ref_path):
        ...
        msg_ref_path.header.frame_id=self.base_frame
    ...
    def pub_pre_path(self,pre_path):
        ...
        msg_pre_path.header.frame_id=self.base_frame
